...
Thread::Finish ()
{
    ...
    // created by exec
    if(tid > 0){
        kernel->ThreadLookupTablebyId.Remove(this);
        Thread* nextThread = NULL;        
        while(waitList.NumInList()){
            Thread* threadPointer = waitList.RemoveFront();
            threadPointer->status = READY;
            kernel->scheduler->ReadyToRun(threadPointer);
            nextThread = threadPointer;
        }
        // returns when it's time for us to run
    }
    // kernel->ThreadLookupTablebyId->Remove()
